...
twitter.trendingClosest = user => new Promise((resolve, reject) => {
	...
	trends.find(user).then(currTrends => {
		...
		if (!currTrends || !util.objCompare(currTrends.location, user.latLng) || (Date.now() - currTrends.timestamp) > 3e+4) {
			debug('trends.find: in the if');
			// Get new trends, save and return them
			if (user.latLng === 'world') {
				debug('trends.find: world');
				return client.get('trends/place', {id: 1}, (err, data) => {
					if (err) return reject(err); // eslint-disable-line curly
					return trends.save(data[0].trends, user).then(resolve).catch(reject);
				});
			}

			return client.get('trends/closest', {lat: user.latLng.latitude, long: user.latLng.longitude}, (err, loc) => {
				if (err) return reject(err); // eslint-disable-line curly
